...
-- Crear tabla de refresh_tokens para revocar sesiones
...
-- Crear tabla de normativa (para persistir secciones del cuerpo normativo)
CREATE TABLE IF NOT EXISTS normativa (
  ...
  archivo_hash VARCHAR(128), -- hash del archivo para verificar integridad
  tamano_archivo INTEGER, -- tamaño en bytes
  usuario_creador VARCHAR(100), -- usuario que subió el archivo
  fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  fecha_actualizacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  activo BOOLEAN DEFAULT TRUE
);

-- Crear índices para mejor rendimiento en normativa
